"""End-to-end lifecycle tests for a guarded braked run."""

import asyncio

from app.compressor import Compressor
from app.entropy import EntropyConfig
from app.models import RunStatus
from app.run_manager import RunManager
from app.worker import Worker
from config.settings import settings


def test_only_authorized_recovery_runs_all_mapped_steps():
    async def exercise():
        entropy_config = EntropyConfig()
        run_manager = RunManager(entropy_config)
        run = await run_manager.create_run("db_migration_loop")
        await run_manager.start_run(run.run_id)
        worker = Worker(run.run_id, run.scenario_id, run_manager, entropy_config, Compressor(settings), (1, 1))
        worker_task = asyncio.create_task(worker.run())
        for _ in range(200):
            if (await run_manager.get_run(run.run_id)).status == RunStatus.braked:
                break
            await asyncio.sleep(0.01)
        assert (await run_manager.get_run(run.run_id)).status == RunStatus.braked
        assert await run_manager.select_fixture_recovery(run.run_id, "additive_email_migration", "Preserve existing data.") == (True, "selected")
        assert await run_manager.authorize_fixture_recovery(run.run_id) == (True, "authorized")
        worker.resume()
        await worker_task
        completed_run = await run_manager.get_run(run.run_id)
        assert completed_run.status == RunStatus.completed
        assert completed_run.meta["recovery_authorized_choice_id"] == "additive_email_migration"
    asyncio.run(exercise())
